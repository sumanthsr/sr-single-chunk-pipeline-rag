...
ChromaDB wrapper for ingestion (write) and retrieval (read).
"""
...
def retrieve(query_vector: list[float], top_k: int | None = None, score_threshold: float | None = None) -> list[dict]:
    rcfg = cfg.retrieval
    ...
    for meta, dist, doc in zip(results["metadatas"][0], results["distances"][0], results["documents"][0]):
        ...
        if score < threshold:
            continue
        output.append({**meta, "score": score, "prefixed_text": doc})
    logger.info(f"Retrieved {len(output)} chunks")
    return output
